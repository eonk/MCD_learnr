...
```{r table, echo = FALSE}
question_text(
  "Let's say you are analysing a variable 'crime_record: previous criminal records of prisoners' using data names 'recidivism'. 
  
  The data has two values 'Yes' and 'No'. To calculate a count of obervation on the valid value of 'crime_record' variable, you want to use the table() function. 
  
  Please enter the code you need to get a count of obsertations (see 2.3.2 in the lab note):",
  
  
  answer("table(crime_record)", message = "Don't forget to start with the data name"),
  answer("table(recidivism$crime_record)", correct = TRUE),
  allow_retry = TRUE
)
```
...
```{r isna, echo = FALSE}
question_radio(
  "After the analysis, you found the 'crime_record' variable has some missing observations. 
  
  To see how many observations are missing for the variable, which code would you use?:",
  
  answer("select(recidivism, crime_record)", message = "nah- this is fo 'variable selection"),
  answer("mean(is.na(recidivism$crime_record))", message = "this will tell you proportion, not count"),
  answer("class(recidivism$crime_record)", message = "think twice"),
  answer("sum(is.na(recidivism$crime_record))", correct = TRUE),
  allow_retry = TRUE
)
```
...
question_radio(
  ...
  Let's say now you are intersted in ONLY prisoners who have crime records, been recorded 'Yes' to the 'crime_record' variable. 
  
  To filter these observation which code would you use:",
  
  answer("mutate(recidivism, crime_record == 'Yes')", message = "nah- this is fo 'variable selection"),
  answer("select(recidivism, crime_record == 'Yes')", message = "this will tell you proportion, not count"),
  answer("filter(recidivism, crime_record == 'Yes')", correct = TRUE),
  answer("summarise(recidivism, crime_record == 'Yes')", message = "think twice"),
  allow_retry = TRUE
)
```